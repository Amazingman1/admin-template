...
export default class BatchFacility extends Vue {
  ...
  private refresh(arr) {
    let allArr: any[] = [];
    arr.forEach((v) => {
      const arr = v.userDevices.map((val) => {
   ...
   * 启用禁用设备
   */
  private hangleFacility(type, handle) {
    this.$confirm(`您确定要执行该操作吗？`, '提示', {
      ...
      let userIds: any[] = [];
      userIds = type === 0 ? this.pc.selectionList : this.moving.selectionList;
   ...
   * 启用停用设备锁
   */
  private hangleLock(type, handle) {
    if (
      this.pc.selectionList.length === 0 &&
      this.moving.selectionList.length === 0
    ) {
      this.$message({
        type: 'warning',
        message: '请选择用户!',
